...
class XadrezSuicoEmparceiradorController extends Controller
{
    ...
    public function export_data($evento_id){
        $user = Auth::user();
        if(Evento::where([["id","=",$evento_id]])->count() > 0){
            $evento = Evento::find($evento_id);
            if (
            ...
            $evento_obj = $evento->export("xadrezsuico-data");
            ...
            // file name that will be used in the download
            $fileName = "Xadrezsuico_evento_" . $evento->id . "_com_dados___" . date("Ymd-His") . "___.xadrezsuico-json";
