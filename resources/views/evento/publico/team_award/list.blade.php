            ...
            <table id="tabela" class="table-responsive table-condensed table-striped" style="width: 100%">
                ...
                <tbody>
                    @foreach($team_award->team_scores()->orderBy("place","ASC")->get() as $score)
                        <tr>
                            <td>{{$score->place}}</td>
                            <td><a href="{{url("/evento/".$event->id."/resultados/enxadrista/".$score->club->id)}}">#{{$score->club->id}}</a></td>
                            <td><a href="{{url("/evento/".$event->id."/resultados/enxadrista/".$score->club->id)}}">{{$score->club->name}}</a></td>
