...
@section('title', 'Grupo de Evento #'.$event_group->id." Pontuação de Times '".$team_award->name."' - Pontuação do Time '".$team->name."'")

@section('content_header')
    <h1>Grupo de Evento #{{$event_group->id}} ({{$event_group->name}}) - Pontuação de Times #{{$team_award->id}} ({{$team_award->name}})</h1>
...
@section('content')
    ...
    <ul class="nav nav-pills">
        <li role="presentation"><a href="{{url("/grupoevento/".$event_group->id."/team_awards/".$team_award->id."/results")}}">Voltar à Lista de Pontuação de Times</a></li>
    </ul>
            ...
            <div class="box-body">
                <table class="table-responsive table-condensed table-striped tabela" style="width: 100%">
                    ...
                    <tbody>
                        ...
                        @php($total = 0)
                        @php($is_points = false)
                        @if($team_award->hasConfig("is_points"))
                            @if($team_award->getConfig("is_points",true))
                                @php($is_points = true)
                            @endif
                        @endif
                                        ...
                                        @if($team_award->hasConfig("limit_places"))
                                            @if($team_award->getConfig("limit_places",true) > $j)
                                                @if(
                                                    ($is_points)
                                                    ?
                                                    $team_award->hasPlace($inscricao->posicao,true,$inscricao)
                                                    :
                                                    $team_award->hasPlace($inscricao->posicao)
                                                )
                                                    {{
                                                        ($is_points)
                                                        ?
