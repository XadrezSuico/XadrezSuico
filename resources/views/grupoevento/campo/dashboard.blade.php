...
@section("title", "Grupo de Evento #".$grupo_evento->id." (".$grupo_evento->name.") >> Dashboard de Campo Personalizado")

@section('content_header')
  <h1>Grupo de Evento #{{$grupo_evento->id}} ({{$grupo_evento->name}}) >> Dashboard de Campo Personalizado</h1>
@stop
...
@section("content")
	...
	<ul class="nav nav-pills">
		<li role="presentation"><a href="/grupoevento/dashboard/{{$grupo_evento->id}}?tab=campo_personalizado">Voltar a Lista de Campos Personalizados na Dashboard de Grupo de Evento</a></li>
	</ul>
	<div class="row">
  ...
  <section class="col-lg-6 connectedSortable">
		...
		<div class="box box-primary">
			...
			<!-- form start -->
			<form method="post" action="{{url("/grupoevento/".$grupo_evento->id."/campos/".$campo->id."/opcao/add")}}">
				<div class="box-body">
					<div class="form-group">
						<label for="name">Nome *</label>
						...
						<tbody>
							@foreach($campo->opcoes->all() as $opcao)
								<tr>
									...
									<td>{{$opcao->value}}</td>
									<td>
										@if($opcao->isDeletavel()) <a class="btn btn-danger" href="{{url("/grupoevento/".$grupo_evento->id."/campos/".$campo->id."/opcao/remove/".$opcao->id)}}" role="button"><i class="fa fa-times"></i></a> @endif
									</td>
								</tr>
							@endforeach
						</tbody>
