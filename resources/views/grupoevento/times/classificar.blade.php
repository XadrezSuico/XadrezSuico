...
@section('title', 'Grupo de Evento > Classificar Premiações de Times')

@section('content_header')
    <h1>Grupo de Evento > Classificar Premiações de Times</h1>
@stop
...
@section('content')
    ...
    <ul class="nav nav-pills">
        <li role="presentation"><a href="{{url("/grupoevento/dashboard/".$grupo_evento->id)}}">Voltar à Dashboard de Grupo de Evento</a></li>
    </ul>

    <div class="box">
        <div class="box-body">
            <ul>
                @foreach($grupo_evento->event_team_awards->all() as $premiacao_time)
    ...
    function execute(i,action){
        switch(action){
            ...
            case 2:
                ...
                $.getJSON('{{url("/grupoevento/premiacao_time/classificar/".$grupo_evento->id."/call")}}'.concat('/').concat(premiacoes_time[i]).concat('/').concat(action),function(data){
                    if(data.ok == 1){
